...
internal class SyncResponseHandler @Inject constructor(
    ...
    suspend fun handleResponse(syncResponse: SyncResponse,
        ...
        Timber.i("SCSCSC-SYNC Start handling sync, is InitialSync: $isInitialSync")

        measureTimeMillis {
            Timber.i("SCSCSC-SYNC check crypto-service started")
            if (!cryptoService.isStarted()) {
                Timber.v("Should start cryptoService")
                Timber.i("SCSCSC-SYNC Should start cryptoService")
                cryptoService.start()
                Timber.i("SCSCSC-SYNC started cryptoService")
            }
            Timber.i("SCSCSC-SYNC pre-onSyncWillProcess")
            cryptoService.onSyncWillProcess(isInitialSync)
            Timber.i("SCSCSC-SYNC post-onSyncWillProcess")
        }.also {
            Timber.v("Finish handling start cryptoService in $it ms")
            Timber.i("SCSCSC-SYNC Finish handling start cryptoService in $it ms")
        }
        ...
        measureTimeMillis {
            ...
            Timber.i("SCSCSC-SYNC Handle toDevice")
            reportSubtask(reporter, InitSyncStep.ImportingAccountCrypto, 100, 0.1f) {
                Timber.i("SCSCSC-SYNC Handle toDevice -01")
                if (syncResponse.toDevice != null) {
                    Timber.i("SCSCSC-SYNC Handle toDevice -02")
                    cryptoSyncHandler.handleToDevice(syncResponse.toDevice, reporter)
                    Timber.i("SCSCSC-SYNC Handle toDevice -03")
                }
                Timber.i("SCSCSC-SYNC Handle toDevice -04")
            }
        }.also {
            Timber.v("Finish handling toDevice in $it ms")
            Timber.i("SCSCSC-SYNC Finish handling toDevice in $it ms")
        }
        Timber.i("SCSCSC-SYNC pre-aggregator")
        val aggregator = SyncResponsePostTreatmentAggregator()
        Timber.i("SCSCSC-SYNC post-aggregator | pre fetchRoot")

        // Prerequisite for thread events handling in RoomSyncHandler
        threadsAwarenessHandler.fetchRootThreadEventsIfNeeded(syncResponse)
        Timber.i("SCSCSC-SYNC post-fetchRootHreaed | pre big transaction")

        // Start one big transaction
        monarchy.awaitTransaction { realm ->
            measureTimeMillis {
                Timber.v("Handle rooms")
                Timber.i("SCSCSC-SYNC Handle rooms")
                reportSubtask(reporter, InitSyncStep.ImportingAccountRoom, 1, 0.7f) {
                    Timber.i("SCSCSC-SYNC Handle rooms -01")
                    if (syncResponse.rooms != null) {
                        Timber.i("SCSCSC-SYNC Handle rooms -02")
                        roomSyncHandler.handle(realm, syncResponse.rooms, isInitialSync, aggregator, reporter)
                        Timber.i("SCSCSC-SYNC Handle rooms -03")
                    }
                    Timber.i("SCSCSC-SYNC Handle rooms -04")
                }
            }.also {
                Timber.v("Finish handling rooms in $it ms")
                Timber.i("SCSCSC-SYNC Finish handling rooms in $it ms")
            }

            measureTimeMillis {
                Timber.i("SCSCSC-SYNC pre-Handle groups")
                reportSubtask(reporter, InitSyncStep.ImportingAccountGroups, 1, 0.1f) {
                    Timber.v("Handle groups")
                    Timber.i("SCSCSC-SYNC Handle groups")
                    if (syncResponse.groups != null) {
                        Timber.i("SCSCSC-SYNC Handle groups -01")
                        groupSyncHandler.handle(realm, syncResponse.groups, reporter)
                        Timber.i("SCSCSC-SYNC Handle groups -02")
                    }
                    Timber.i("SCSCSC-SYNC Handle groups -03")
                }
            }.also {
                Timber.v("Finish handling groups in $it ms")
                Timber.i("SCSCSC-SYNC Finish handling groups in $it ms")
            }

            measureTimeMillis {
                Timber.i("SCSCSC-SYNC pre-accountData")
                reportSubtask(reporter, InitSyncStep.ImportingAccountData, 1, 0.1f) {
                    Timber.v("Handle accountData")
                    Timber.i("SCSCSC-SYNC Handle accountData -01")
                    userAccountDataSyncHandler.handle(realm, syncResponse.accountData)
                    Timber.i("SCSCSC-SYNC Handle accountData -02")
                }
            }.also {
                Timber.v("Finish handling accountData in $it ms")
                Timber.i("SCSCSC-SYNC Finish handling accountData in $it ms")
            }

            measureTimeMillis {
                Timber.v("Handle Presence")
                Timber.i("SCSCSC-SYNC presence -01")
                presenceSyncHandler.handle(realm, syncResponse.presence)
                Timber.i("SCSCSC-SYNC presence -02")
            }.also {
                Timber.v("Finish handling Presence in $it ms")
                Timber.i("SCSCSC-SYNC Finish handling Presence in $it ms")
            }
            Timber.i("SCSCSC-SYNC pre sync-save-token")
            tokenStore.saveToken(realm, syncResponse.nextBatch)
            Timber.i("SCSCSC-SYNC post sync-save-token")
        }

        // Everything else we need to do outside the transaction
        Timber.i("SCSCSC-SYNC pre-aggregator-handle")
        aggregatorHandler.handle(aggregator)
        Timber.i("SCSCSC-SYNC post-aggregator-handle")

        syncResponse.rooms?.let {
            Timber.i("SCSCSC-SYNC check-push -01")
            checkPushRules(it, isInitialSync)
            Timber.i("SCSCSC-SYNC check-push -02 / sync invite with server")
            userAccountDataSyncHandler.synchronizeWithServerIfNeeded(it.invite)
            Timber.i("SCSCSC-SYNC check-push -03 / synced invite with server")
            dispatchInvitedRoom(it)
            Timber.i("SCSCSC-SYNC check-push -04 / dispatced invite")
        }
        Timber.i("SCSCSC-SYNC sync response rooms handled")
        syncResponse.groups?.let {
            Timber.i("SCSCSC-SYNC pre-schedule group data fetching")
            scheduleGroupDataFetchingIfNeeded(it)
            Timber.i("SCSCSC-SYNC post-schedule group data fetching")
        }

        Timber.v("On sync completed")
        Timber.i("SCSCSC-SYNC On sync completed")
        cryptoSyncHandler.onSyncCompleted(syncResponse)
        Timber.i("SCSCSC-SYNC On sync completed | cryptohandler knows")

        // post sync stuffs
        monarchy.writeAsync {
            Timber.i("SCSCSC-SYNC pre-write-monarchy")
            roomSyncHandler.postSyncSpaceHierarchyHandle(it)
            Timber.i("SCSCSC-SYNC post-write-monarchy")
        }
        Timber.i("SCSCSC-SYNC finally done")
    }
